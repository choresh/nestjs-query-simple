...
const TAR_FILE_PATH = "./codeGeneration/generatedSchema.graphql";
...
class SchemaGenerator {
  ...
  private static _findResolverClasses(
    srcDirPath: string,
    node: ts.Node,
    ...
    if (
      ...
      const resolverClass = Object.values(
        require(resolverFilePath),
      )[0] as Function;
      resolverClasses.push(resolverClass);
    }
    ts.forEachChild(node, (currNode) => {
      this._findResolverClasses(srcDirPath, currNode, resolverClasses);
    }); // Recursion!
  }
